...
- **AppImage**: 92MB (deps bundled into main process, no node_modules shipped)
...
npm run package:appimage # Build AppImage (~92MB)
...
- **Classifier**: `src/renderer/gestures/classifier.ts` — Hybrid angle+distance fingerCurl, squared-distance pinch detection, `computeHandFlatness()`, pre-allocated curls buffer. Priority: Fist → Pinch → L-Shape → Point → FlatDrag → OpenPalm
...
- electron-builder `files` whitelist: `dist/main`, `dist/preload`, `dist/renderer`, `native/.node`

## Performance

- **Gesture throughput**: ~1.2M frames/sec (39,800x headroom at 30fps input)
